...
if ($cek_data && $cek_data["DATA"] == "inhouse" && $zeroQty > 0) {
	echo '<div style="background:#fff3cd;color:#856404;padding:15px;margin:15px 0;border:2px solid #ffc107;border-radius:8px;font-weight:bold;font-size:16px;">⚠️ PERINGATAN: Terdapat ' . $zeroQty . ' item dengan QTY = 0. Periksa kembali data sebelum mencetak!</div>';
}
?>
<div>
	<table id="" class="table table-bordered" style="width:85%;margin-left: 60px;/*margin-top:5mm*/">
		<tbody>
			<?php
			...
			if ($dataType == "inhouse") {
				foreach($rows as $r) {
					...
					if ($r['BUILDING'] != "") {
						$bg = "bg-" . preg_replace('/[^a-zA-Z0-9_-]/', '', $r["BUILDING"]);
					} else {
						$bg = "bg-B-GRADE";
					}
			?>
						...
						?>
							<div class="row" style="margin:0px;height:70px">
								...
								<div class="col-xs-8" style="padding:0;text-align: left;">
									...
									<h6 class="item">QTY<text style="color:white !important">----</text>: <?php if ($r['QTY'] != "") {
																												echo number_format($r['QTY']);
																											} ?></h6>
...
<style type="text/css">
	...
	.bg-A {
		background: rgb(108, 166, 81) !important;
